...
<head>
    ...
    <title>Login Portal - {{ config('aplikasi.nama_aplikasi', 'Mitra Usaha 2') }}</title>
    ...
    <style>
        ...
        /* Menghindari teks input tertimpa ikon mata */
...
<body class="antialiased bg-white text-gray-900 selection:bg-brand-red selection:text-white font-sans h-screen overflow-hidden">
        ...
        <div class="hidden lg:flex w-1/2 relative items-center justify-center overflow-hidden bg-gray-100">
            ...
            <div class="relative z-10 glass-card rounded-[2rem] p-12 flex flex-col items-center text-center max-w-md mx-8 transition-transform hover:scale-[1.02] duration-500">
                ...
                <h1 class="text-4xl font-black text-white tracking-tight mb-3 drop-shadow-md">
                    Portal Internal<br>Mitra Usaha 2
                </h1>
                
                <p class="text-white/90 text-sm font-medium leading-relaxed drop-shadow-sm">
                    Manajemen data terpusat untuk efisiensi transaksi, akurasi stok gudang, dan operasional harian.
                </p>
        ...
        <div class="w-full lg:w-1/2 flex flex-col relative bg-white px-8 sm:px-16 md:px-24 overflow-y-auto form-scroll py-10 lg:py-0">
            ...
            <div class="w-full max-w-md mx-auto my-auto">
                ...
                <div class="mb-8 text-center lg:text-left mt-4 lg:mt-0">
                    ...
                    <h2 class="text-3xl sm:text-4xl font-black text-brand-dark tracking-tight mb-2">Selamat Datang.</h2>
                    <p class="text-gray-500 font-medium text-sm sm:text-base">Silakan masuk menggunakan kredensial Anda.</p>
                </div>

                <x-auth-session-status class="mb-6 bg-green-50 text-green-600 p-4 rounded-xl text-sm font-bold border border-green-100" :status="session('status')" />
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4 sm:space-y-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="password" type="password" name="password" required autocomplete="current-password" placeholder="••••••••" 
                                class="input-premium block w-full pl-11 pr-12 py-3 bg-gray-50 border border-gray-200 text-gray-900 rounded-xl transition-all duration-300 outline-none placeholder-gray-400 font-medium">
                            
                            {{-- Tombol Mata (Hold to Show) --}}
                            <button type="button" id="togglePasswordBtn" class="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-brand-red transition-colors focus:outline-none cursor-pointer">
                    ...
                    <div class="pt-6">
                        <button type="submit" class="epl-btn w-full bg-brand-red text-white font-black text-sm tracking-widest uppercase py-4 rounded-xl shadow-lg shadow-brand-red/20 border border-transparent mb-4">
                            <span>{{ __('Masuk Sistem') }}</span> <i class="fas fa-sign-in-alt epl-icon-slide ml-1"></i>
                        </button>
                    </div>
                </form>
    ...
    {{-- SCRIPT UNTUK EFEK HOLD-TO-SHOW PASSWORD --}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Event saat mouse ditekan & dilepas
            eyeBtn.addEventListener('mousedown', showPwd);
            eyeBtn.addEventListener('mouseup', hidePwd);
            eyeBtn.addEventListener('mouseleave', hidePwd); // Antisipasi kursor digeser menjauh saat menekan

            // Event untuk layar sentuh (HP/Tablet)
            eyeBtn.addEventListener('touchstart', showPwd);
            eyeBtn.addEventListener('touchend', hidePwd);
        });
    </script>
</body>
</html>