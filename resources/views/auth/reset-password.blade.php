...
<head>
    ...
    <title>Atur Ulang Sandi - {{ config('aplikasi.nama_aplikasi', 'Mitra Usaha 2') }}</title>
...
<body class="antialiased bg-white text-gray-900 selection:bg-brand-red selection:text-white font-sans h-screen overflow-hidden">
    ...
    <div class="flex w-full h-full">
        ...
        <div class="hidden lg:flex w-1/2 relative items-center justify-center overflow-hidden bg-gray-100">
            ...
            <div class="relative z-10 glass-card rounded-[2rem] p-12 flex flex-col items-center text-center max-w-md mx-8 transition-transform hover:scale-[1.02] duration-500">
                ...
                <h1 class="text-4xl font-black text-white tracking-tight mb-3 drop-shadow-md">
                    Keamanan Akun<br>Mitra Usaha 2
                </h1>
                
                <p class="text-white/90 text-sm font-medium leading-relaxed drop-shadow-sm">
                    Buat kata sandi baru yang kuat dan unik untuk mengamankan akses ke dalam sistem operasional perusahaan.
                </p>
        ...
        <div class="w-full lg:w-1/2 flex flex-col justify-center relative bg-white px-8 sm:px-16 md:px-24">
            ...
            <a href="{{ route('login') }}" class="absolute top-8 left-8 sm:left-12 text-sm font-bold text-gray-400 hover:text-brand-red transition-colors flex items-center gap-2 group w-fit">
            ...
            <div class="w-full max-w-md mx-auto mt-12 lg:mt-0">
                ...
                <div class="mb-8 text-center lg:text-left mt-4 lg:mt-0">
                    ...
                    <h2 class="text-3xl sm:text-4xl font-black text-brand-dark tracking-tight mb-2">Buat Sandi Baru.</h2>
                    <p class="text-gray-500 font-medium text-sm sm:text-base">Silakan buat kata sandi baru untuk akun Anda.</p>
                </div>
                ...
                <form method="POST" action="{{ route('password.store') }}" class="space-y-5">
                    ...
                    <div>
                        <label for="email" class="block font-bold text-xs text-gray-500 mb-1.5 uppercase tracking-widest">Alamat Email Terdaftar</label>
                        <div class="relative">
                            ...
                            <input id="email" type="email" name="email" value="{{ old('email', $request->email) }}" required autofocus autocomplete="username" readonly
                                class="input-premium block w-full pl-11 pr-4 py-3.5 bg-gray-100 border border-gray-200 text-gray-600 rounded-xl outline-none font-medium cursor-not-allowed">
                        </div>
                        <x-input-error :messages="$errors->get('email')" class="mt-2 text-sm text-brand-red font-semibold" />
                    </div>

                    <div>
                        <label for="password" class="block font-bold text-xs text-gray-500 mb-1.5 uppercase tracking-widest">Kata Sandi Baru</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400">
                    ...
                    <div>
                        <label for="password_confirmation" class="block font-bold text-xs text-gray-500 mb-1.5 uppercase tracking-widest">Konfirmasi Sandi Baru</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400">
                        ...
                        <x-input-error :messages="$errors->get('password_confirmation')" class="mt-2 text-sm text-brand-red font-semibold" />
                    </div>
                    ...
                    <div class="pt-4">
                        <button type="submit" class="epl-btn w-full bg-brand-red text-white font-black text-sm tracking-widest uppercase py-4 rounded-xl shadow-lg shadow-brand-red/20 border border-transparent">
                            <span>{{ __('Atur Ulang Kata Sandi') }}</span> <i class="fas fa-save epl-icon-slide ml-1"></i>
                        </button>
                    </div>
                </form>
